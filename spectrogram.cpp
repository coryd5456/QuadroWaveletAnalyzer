...
#include <iostream>
//#include "fftcuda.h"
///  This is the constructer of the spectragram
///  This is the code for grabbing data and computing the FFT
///
///
///

//constructor of the spectrogram object
Spectrogram::Spectrogram(unsigned int _sampleRate,
    ...
    ringBufferInd = 0;
    //this is the size of our bucket
    fftSize = 4096;
...
// Need to determine how this is doing an FFT on the data itself.
unsigned int
Spectrogram::processData(float *buffer,
    ...
    //Grabs max value from the 128 data bucket measurment
    for (unsigned int bufferInd = 0; bufferInd < bufferLength; bufferInd++) {
        ...
        //stores value in waveRingBuffer to be used for FFT
        waveRingBuffer[ringBufferInd] = value;
        ringBufferInd = (ringBufferInd + 1) % ringBufferSize;
        sampleCounter++;
        //once number of samples is the size of our fft, computes fft for new line.
        if (sampleCounter == fftSize) {
            sampleCounter -= samplesPerLine;

            newLines++;
//This is where the fft is being defined and created.
...
#else
            //computes the FFT on the raw data here?
...
//This is where the FFT is being defined
void

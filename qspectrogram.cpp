...
//Qspectrogram takes in a spectrogram and converts it into a widget
//constructor
//Layout Vertical needs to be flipped such that the waterfall plot is from top to bottom
// go through drawColorbarPlot to learn more about drawing with QPainter
// my guess is that you set an image space,
// then draw to that image space
QSpectrogram::QSpectrogram(Spectrogram *_spectrogram,
    ...
    //What are the enums for drawMode?
    drawMode = DRAWMODE_SCROLL;
    //What are the enums for the layout?
    //LAYOUT_VERTICAL
    //LAYOUT_HORIZONTAL
    ...
    logScaleFreq = true;
    logScaleAmpl = true;
    //The color of the spectragram is stored in a color list of size 5.
...
//drawPixmap draws an images to a location.
//the image is the pixmap which is created in refreshPixmap().
//pixmap is a private variable in the class header
//pixmap is a Qpixmap datatype (learn what this is?)
...
//This function handles the scaling of the widget when the window changes size due to the user
void
...
//This function tells the drawGrid where to draw the frequency grid lines for painter drawLine
int
...
//This function tells the drawGrid where to draw the time grid lines for painter drawLine
int
...
//paints a grid when called?
//paints the log-log time-freq plot
...
//This function draws the raw audio waveform with nothing done to it
//Might be a good dive
//I need to learn how std::list<float>::iterator works
void
QSpectrogram::drawWaveformPlot(QPainter &painter) {
    ...
    //for (;;) {
    while ( itMin != spectrogram->waveEnvelopeMin.begin() &&
            itMax != spectrogram->waveEnvelopeMax.begin() &&
            pixel >= plotx ) {
        //if (itMin == spectrogram->waveEnvelopeMin.begin()) break;
        //if (itMax == spectrogram->waveEnvelopeMax.begin()) break;
        //if (pixel < plotx) break;

        // Grabbing min and max value from bucket of 128 voltage measurments from pulse audio library in
        // spectragram and pulsethread
        int minValue = (int)(0.5 * ((float)waveformWidth)*(*itMin));
        int maxValue = (int)(0.5 * ((float)waveformWidth)*(*itMax));

        //draws line based off of the min and max value in a bucket of 128 raw voltage data points.
        if (layoutMode == LAYOUT_HORIZONTAL) {
        ...
        }// so this has 2 time values per line. rather than drawing a line to interpolate time values.

        pixel--; //moves the lines all back one.
        itMin--; // learn iterators
        itMax--; //
    }
}


//This function draws the fft on the side of the waterfall plot
void
QSpectrogram::drawSpectrumPlot(QPainter &painter) {
    if (spectrogram->spectrogramData.empty()) return;

    //spectragram grabs the data this throws it into a vector bucket.
    ...
    thinPen.setWidth(1);
    thinPen.setColor(gridColor);
    //thinPen.setColor(QColor(Qt::red));
    painter.setPen(thinPen);
...
// This is where the waterfall plot gets rendered from spectragram
// I should read through each line of this to understand how it's rendered
// What I don't really understand is how each of the functions that paint something
// add to the painting

// uses set pixel to draw the spectragram pixel by pixel as data comes in from spectrogram.cpp
// I need to unpack this giant for loop from setpixel to figure out how it populates the pixels with data
// I could probably make my own plotter using QPainter and setPixel
void
QSpectrogram::renderImage(unsigned int newLines, bool redraw) {

    if (image) {
        QImage *oldimage = image;
        //Creates image template?
        image = new QImage(plotwidth, plotheight, QImage::Format_RGB32);
        //fills every pixel with the background color
    ...
    //The newLines are the number of data points?
    // so, number of new data points for the new line
    if (redraw) {
    ...
    //This is such a flex of a for loop
                    ...
                    if (y < prevy && y > 0 && x >= 0 && x < (int)plotwidth && prevy < (int)plotheight) {
                        ...
                        if (value > 0.01) {
                            ...
                            for (int yi = y; yi < prevy; yi++) {
                                ...
                                //int looks like( FF (red byte)(green byte)(blue byte))

                                if (yi >= 0 && yi < (int)plotheight)
                                //set pixel is how the spectragram gets drawn, and I can use setPixel to programatically
                                //draw an image by pixel. (coordinates, color)
                                //The color is an int that is generated by qRGB which is the RGB value bytes together side by side
...
//draws the color bar on the side which shows the legend for what the amplitude in dB
//coorisponds to in color
void
QSpectrogram::drawColorbarPlot(QPainter &painter) {
    //Main question to answer
    //How does it know where to draw the box
    // looks like I need to look up where plotx + plotwidth +spectrum width get defined.
    int r, g, b;
    QPen colorPen;
    ...
    for (unsigned int y = ploty; y < ploty + plotheight; y++) {
        ...
        if (drawSpectrum) {
            //This draws the colors inside the box
            painter.drawLine(startX + colorBarWidth/2, y, startX + colorBarWidth - 5, y);
        }
    }
    //The following code draws the box itself
    colorPen.setColor(gridColor);
    painter.setPen(colorPen);
    //makes the outline of the box
    ...
    for (int logIt = logMinInt; logIt <= logMaxInt; logIt++) {
        ...
        if (value >= minAmpl && value <= maxAmpl) {
            ...
            //draws the lines marking where the dB values are
...
//This is where the image for the pixmap is created that is used in the paint event
void
QSpectrogram::refreshPixmap() {
    ...
    if (image) {
        //note that this is using Qpainter and is the structure fed into the other
        //functions plotting the image
        //This is the waterfall plot of the spectragram
        painter.drawImage(plotx, ploty, *image);
    }
    if (drawSpectrum) {
        //This draws the FFT on the side of the waterfall plot
        drawSpectrumPlot(painter);
    }
    if (drawWaveform) {
        //This draws the audio waveform directly from the microphone
        drawWaveformPlot(painter);
    }
    if (drawColorbar) {
        //This is the color map for the log-log plot does not decide the color?
        //just draws the legend
        drawColorbarPlot(painter);
    }
    //This is just the time-freq log-log plot
    drawGrid(painter);
    //This is where it redraws everything every frame.
    update();
}
...
//This is where the data gets processesed.
//So, I just need to load data into the buffer.
...
QSpectrogram::processData(float *buffer, unsigned int bufferLength) {
    ...
    //Every time process Data is called it re-draws the screen
